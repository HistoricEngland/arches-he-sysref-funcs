...
class TestGenerateUniqueReferencesFunction(TransactionTestCase):
    ...
    def setUp(self):
        ...
        graph = Graph.objects.get(
            graphid=self.test_model_graph_id)
        graph.publish(user=admin)

        # Import second_test_model graph
        with open(
            os.path.join(
                "tests/fixtures/resource_graphs/second_test_model.json"), "r"
        ) as f:
            archesfile = JSONDeserializer().deserialize(f)
        resource_graph_importer(archesfile["graph"])
        graph = Graph.objects.get(
            graphid=self.second_test_model_graph_id)
        graph.publish(user=admin)
    ...
    def create_and_assert_resource(
        ...
        resource_uuid = resource.get_node_values(
            "ResourceID")[0].get("en", {}).get("value")

        self.assertEqual(str(resource.resourceinstanceid), resource_uuid)

        if assert_prn:
            self.assertEqual(
                TestGenerateUniqueReferencesFunction.prn + self.initial_seed, resource_prn)
    ...
    def test_02_graphs_exist(self):
        ...
        graph1 = Graph.objects.filter(
            graphid=self.test_model_graph_id)
        graph2 = Graph.objects.filter(
            graphid=self.second_test_model_graph_id)
    ...
    def test_03_create_new_test_model_with_description(self):
        ...
        tile_data = {
            description_node_id: {
                "en": {
                    "value": "Test Resource with description",
                    "direction": "ltr"
                }
            }
        }
        self.create_and_assert_resource(
            self.test_model_graph_id, tile_data, description_nodegroup_id, assert_prn=True
        )
    ...
    def test_05_create_test_model_without_description(self):
        tile_data = {
            '7a9d162c-63f0-11f0-9f7e-460d1d596ee6': {'en': {'direction': 'ltr', 'value': ''}},
        ...
        self.create_and_assert_resource(
            self.test_model_graph_id, tile_data, nodegroup_id, self.prn + self.initial_seed
        )
    ...
    def test_07_create_second_test_model_without_description(self):
        tile_data = {
            'b4d8a7a4-624a-11f0-8f24-96a8a23bc0be': {'en': {'direction': 'ltr', 'value': ''}},
    ...
    def test_08_create_another_second_test_model_with_description(self):
        ...
        self.create_and_assert_resource(
            self.second_test_model_graph_id, tile_data, description_nodegroup_id, self.prn +
            self.initial_seed
        )
    ...
    def test_09_invalid_resourceid(self):
        ...
        tile_data = {
            'b4d8a7a4-624a-11f0-8f24-96a8a23bc0be': {'en': {'direction': 'ltr', 'value': 'This is NOT a valid resource UUID'}},
            'eba9e4aa-624a-11f0-8f24-96a8a23bc0be': None,
            '61f674ee-624a-11f0-8f24-96a8a23bc0be': None,
            'fd70c92a-6249-11f0-8f24-96a8a23bc0be': None,
